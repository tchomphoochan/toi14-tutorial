...
ิอย่างไรก็ตาม วิธีนี้ค่อนข้างจะช้าในเชิงของ constant factor เนื่องมาจากธรรมชาติของ recursive function จึงมีอีกวิธีหนึ่งที่สามารถหาคำตอบใน Big O เดียวกัน แต่ด้วย constant factor ที่น้อยกว่า
...
อย่างแรกเลยคือเราต้องหาว่า state เราจะบ่งบอกถึงอะไร เราก็ถามตัวเองว่า ในโจทย์ข้อนี้ เขามีข้อกำหนดอะไรมาให้เราบ้าง? ในที่นี้โจทย์เอิ่ยถึงสองอย่างที่น่าสนใจคือจำนวณของเหรียญกับผลรวมค่าของเหรียญ ถ้าเราลองใช้จำนวณของเหรียญเป็น state แล้ว `dp[จำนวณเหรียญ]` จะบ่งบอกถึงอะไรละ? ผลรวมน้อยสุดที่เป็นไปได้? ทุกผลรวมที่เป็นไปได้? ทั้งสองอย่างนี้ไม่เหมาะสมกับคำตอบที่เราอยากได้เลย เพราะอันแรกไม่สามารถนำไปใช้ได้ และอันที่สองดูจะใช้เวลามากเกินไป ณ จุดนี้เราไม่รู้เลยว่าจะไปต่อยังไงเพราะว่าเราไม่สามารถนิยาม `dp[จำนวณเหรียญ]` ให้เหมาะสมกับโจทย์ได้ ดังนั้นเราก็ต้องถอยกลับไปลองใช้ผลรวมค่าของเหรียญมาเป็น state

ถ้าใช้ผลรสมค่าของเหรียญจะเกิดอะไรขึ้นละ? อย่างแรกเลยเราก็สามารถที่จะนิยาม `dp[ผลรวมค่าของเหรียญ]` ให้เท่ากับคำตอบเลย ซึ่งคือจำนวณเหรียญน้อยสุดที่เป็นไปได้ สิ่งสุดท้ายที่เราต้องหาคือจะคำนวณคำตอบของ state `dp[ผลรวมค่าของเหรียญ]` จาก state ใหนได้บ้าง กล่าวอักนัยหนึ่งคือ จาก state ก่อนหน้า เราจะมาหา state นี้ได้อย่างไร ถ้าเรามีผลรวมค่าของเหรียญเท่ากับ `y` และต้องการให้มันเท่ากับ `x` ที่แน่นอนคือ `x = y + C[i]` เนื่องจากเราต้องใช้หนึ่งเหรียญในการเพิ่มผลรวมค่าของเหรียญ ถ้าเราย้ายข้างสมการ เราก็จะได้ว่า `y = x - C[i]` โดยที่ `C[i]` นั้นเป็นค่าของเหรียญทชนิดหนึ่งที่ให้มาในโจทย์ มากไปกว่านั้น สำหรับ `x` ใดๆ เราสามารถใช้ทุก `i` ที่เป็นไปได้เพื่อหาแต่ละค่า `y`  
...
int coin_change(int x) {
	...
	visited[x] = true;
	dp[x] = INF;
	for(int i = 1; i <= n; i++) if(C[i] <= x) dp[x] = min(solve(x - C[i]) + 1, dp[x]);
	return dp[x];
}
int solve(int k) {
	return coin_change(k);
}
```
...
int coin_change(int k) {
	...
	for(int x = 1; x <= k; x++) for(int i = 1; i <= n; i++) {
		if(C[i] <= x) dp[x] = min(dp[x - C[i]] + 1, dp[x]);
	}
	return dp[k];
}
```
...
> กำหนดจำนวนเต็ม n และตาราง nxn เหรียกว่า a[i][j] เราจะเริ่มจากช่องที่ a[1][1] และเดินไปยัง a[n][n] โดยที่จะสามารถขยับได้สองทางจาก a[i][j] คือไปยัง a[i+1][j] และ a[i][j+1] เท่านั้น หลังจากที่เดินเสร็จแล้ว เราจะหาผลรวมของ a[i][j] ที่ได้ใช้เดิน ถามว่าผลรวมมากที่สุดคือเท่าใด
...
```
อัลกอริทึมนี้วิเคราะห์ big O ได้ง่ายๆเลยเหนื่องจากเป็นแค่ for loop สองชั้น เท่ากับ `O(n^2)`